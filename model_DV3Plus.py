...
class BilinearUpsampling(Layer):
    ...
    def call(self, inputs):
        if self.upsampling:
            return K.tf.image.resize_bilinear(inputs, (int(inputs.shape[1] * self.upsampling[0]),int(inputs.shape[2] * self.upsampling[1])),align_corners=True)
        else:
            return K.tf.image.resize_bilinear(inputs, (self.upsample_size[0],self.upsample_size[1]),align_corners=True)
